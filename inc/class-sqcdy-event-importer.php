...
class SQCDY_Event_Importer {
	 ...
	 */
	public static function import_from_feed( $feed_url, $options = array() ) {
		$defaults = array(
		...
		if ( ! empty( $options['default_category'] ) ) {
			...
			if ( $category_term ) {
				$post_data['tax_input'] = array(
					'events-category' => array( $category_term->term_id ),
				);
			}
		}
	...
	private static function set_acf_fields( $post_id, $ics_event, $options ) {
		...
		if ( isset( $ics_event['dtstart']['timestamp'] ) ) {
			...
			$start_date = wp_date( 'F j, Y', $start_timestamp );
			...
			if ( ! $is_all_day && ! $is_multi_day ) {
				$start_time = wp_date( 'g:i a', $start_timestamp );
				...
				if ( isset( $ics_event['dtend']['timestamp'] ) ) {
					...
					$end_date      = wp_date( 'F j, Y', $end_timestamp );
				...
				if ( isset( $ics_event['dtend']['timestamp'] ) ) {
					...
					$end_date      = wp_date( 'F j, Y', $end_timestamp );
					$end_time      = wp_date( 'g:i a', $end_timestamp );

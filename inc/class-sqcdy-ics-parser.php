...
class SQCDY_ICS_Parser {
	...
	public static function parse_ics_content( $ics_content ) {
		...
		foreach ( $lines as $line ) {
			...
			if ( isset( $previous_line ) && ( 0 === strpos( $line, ' ' ) || 0 === strpos( $line, "\t" ) ) ) {
	...
	private static function parse_datetime( $value, $params = '' ) {
		$is_date_only = false !== strpos( $params, 'VALUE=DATE' );
		$timezone     = null;
	...
	public static function is_multi_day_event( $event ) {
		...
		$start_date = gmdate( 'Y-m-d', $event['dtstart']['timestamp'] );
		$end_date   = gmdate( 'Y-m-d', $event['dtend']['timestamp'] );
